* ありは自分で考えるのが得意なように思えるから、C と異なる部分を大まかに説明してみた。細かいところは脳内補完で、、
* C# は簡単だけど、C の知識があることで逆に間違えやすいところがあると思う。
* これは、昨日のコードが動くように訂正できるようにするためだけに書き出したもの。（言葉の使い方が文法的に間違ってるところがあるかも、、）
* 長い文章に見えるけど、ほとんどがサンプルコード。中身はスカスカなので気楽にどうぞ。

# まずは動くものを作る
　昨日のファイルは後で直すとして、新規プロジェクトを作成。
...
　C# では、とにかく名前を決めて、メモリを確保して、それを渡したり操作したりする、という感覚が必要。位置を指定するためだけに、 **「位置という情報を作成する」** 必要がある。慣れるまで、ちょっとひっかかるかも。。
```
