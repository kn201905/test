...
{
	string a;
	text_box.AppendText(a);
}
```
　**変数「a」の初期化がなされていない**（＝「a」に何の情報も設定されていない）のに、AppendText() で利用されることはおかしい、と判定される。

　C# では、新しく作った変数が初期化されていない場合、その変数は利用できない、という制限がかかっている。現時点では、変数の初期化を行うのが new である、と理解しておくと良い。

　`string a = "Hello, ";` で、a が初期化されているため、`new` が省略できる、と思っておくと良い。
...
　上の例で、`Test test = new Test();` とすると、p に１、q に２が代入された test が生成される。
