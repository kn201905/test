...
* c を表示するときに、`text_box.AppendText(c);` としたいところであるけれど、`AppendText()` は文字列しか受け取ることができない。
...
# class Test をプログラムに組み込んでみる

　以下の２つのことをしてみる。

* Form1.cs に class Test を書き加えてみる。
* Form1 の中に、Test オブジェクト（＝もの）を組み込む。
